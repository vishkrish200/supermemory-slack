...
function executeCommand(command: string): string | { error: true; message: string } {
    ...
    try {
        ...
        return { error: true, message: err.stdout || err.stderr || "Unknown error" };
    }
}
...
async function promptForEnvVars() {
    ...
    const _devVarsExamplePath = path.join(__dirname, "..", ".dev.vars.example");

    if (!fs.existsSync(devVarsPath)) {
        console.log("\x1b[33mNow, let's set up your environment variables.\x1b[0m");
        
        const vars = {
            AUTH_GITHUB_ID: await prompt("Enter your GitHub Client ID (enter to skip)", ""),
            AUTH_GITHUB_SECRET: await prompt("Enter your GitHub Client Secret (enter to skip)", ""),
            BETTER_AUTH_URL: await prompt("Enter your Better Auth URL", "http://localhost:8787"),
            SECRET: generateSecureRandomString(32),
            LEMONSQUEEZY_CHECKOUT_LINK: await prompt("Enter your Lemonsqueezy Checkout Link (enter to skip)", ""),
        };
        ...
        try {
            ...
            console.log("\x1b[33m.dev.vars file created with environment variables.\x1b[0m");
        } catch (error) {
...
async function main() {
    try {
        ...
        console.log("\x1b[33mYou can now run 'bun run dev' to start the development server.\x1b[0m");
    } catch (error) {
        console.error("\x1b[31mError:", error, "\x1b[0m");
        cancel("Operation cancelled.");
        throw error;
