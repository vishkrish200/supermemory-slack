...
export const slackToken = sqliteTable("slackToken", {
  ...
  encryptedToken: text("encryptedToken").notNull(), // Base64 encoded IV + ciphertext
  encryptionAlgorithm: text("encryptionAlgorithm")
    .notNull()
    .default("AES-GCM-256"),
  keyId: text("keyId").notNull().default("default"), // For key rotation support
...
export const securityAuditLog = sqliteTable("securityAuditLog", {
  id: text("id").primaryKey(),
  eventType: text("eventType").notNull(), // token_created, token_rotated, token_revoked, gdpr_delete, etc.
  teamId: text("teamId"), // Optional: related team ID (non-PII)
  tokenId: text("tokenId"), // Optional: related token ID (non-PII)
  actorType: text("actorType").notNull(), // system, admin, user
  actorId: text("actorId"), // Optional: non-PII identifier of actor
  details: text("details"), // JSON string with non-sensitive operation details
  ipAddress: text("ipAddress"), // IP address of the request (if applicable)
  userAgent: text("userAgent"), // User agent string (if applicable)
  success: integer("success", { mode: "boolean" }).notNull(),
  errorMessage: text("errorMessage"), // Error details (never containing sensitive data)
  createdAt: integer("createdAt", { mode: "timestamp" }).notNull(),
});
