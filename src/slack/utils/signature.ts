...
export function validateSlackEvent(payload: unknown): payload is SlackEvent {
  ...
  if (p.type === "event_callback") {
    ...
    const hasTeamId = !!p.team_id;
    const hasApiAppId = !!p.api_app_id;
    const hasEvent = !!(p.event && typeof p.event === "object");
    const hasEventType =
      hasEvent && !!(p.event as Record<string, unknown>).type;
    ...
    console.log("🔍 Event validation debug:", {
      ...
      hasEventType,
      teamId: p.team_id,
      apiAppId: p.api_app_id,
      eventType: hasEvent ? (p.event as Record<string, unknown>).type : null,
      allKeys: Object.keys(p),
    });

    // Relax validation - api_app_id might not always be present
    return !!(
      p.team_id &&
      p.event &&
