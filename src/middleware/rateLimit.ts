...
export const ratelimiter = async (
  ...
  // Temporarily disable rate limiting while auth is simplified
  // TODO: Re-enable once auth system is properly configured
  await next();
  return;
...
export const createRateLimiter = <_T extends { id: string }>(
  ...
  return async (c, next) => {
    ...
    if (!existing) {
      ...
      throw new HTTPException(429, {
        message: `Rate limit exceeded ${JSON.stringify(
          tierLimit
    ...
    if (existing) {
      ...
      c.header(
        ...
        (tierLimit.limit - existing.count - 1).toString()
      );
      c.header(
        "X-RateLimit-Reset",
